  ...
  const handleSearch = async () => {
    ...
    if (tripType == "oneWay" && departureDate) {
      ...
      const response = await fetch(
        "http://localhost:8080/api/detailrecommendations",
        {
          method: "POST",
          headers: {
          ...
          <div className="suggestions-container">
            {fromSuggestions?.map((suggestion, index) => (
              <div
                ...
                onClick={() =>
                  handleSelection(
                    { name: suggestion.name, code: suggestion.code },
                    "from"
                  )
                }
                key={index}
          ...
          <div className="suggestions-container">
            {toSuggestions?.map((suggestionT, index) => (
              <div
                ...
                onClick={() =>
                  handleSelection(
                    { name: suggestionT.name, code: suggestionT.code },
                    "to"
                  )
                }
                key={index + "T" + suggestionT.code}
              >
